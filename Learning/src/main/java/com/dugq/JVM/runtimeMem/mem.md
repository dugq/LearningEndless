...
| major gc   | 老年代         | 老年代满或者已使用一定比例 | CMS独有的概念，针对老年代独立回收 详见[CMS](./G1VsCms.md)    |
| mixed gc   | 新生代+部分老年代   | 老年代满或者已使用一定比例 | G1 独有的概念，和full gc 不尽相同,详见[G1](./G1VsCms.md) |
| full gc    | 新生代+老年代+方法区 | 老年代满或者已使用一定比例 | 详见[full gc](./fullgc.md)                    |
| 串行 GC      | -           | -             | GC时STW，单线程进行标记-回收                           |
| 并行GC       | -           | -             | GC时STW，多线程进行标记-回收                           |
| 并发GC       | -           | -             | GC时部分STW，多线程进行标记-回收                         |                                          
